...
function createCompiler(config) {
  ...
  compiler.hooks.done.tap('finished-log', stats => {
    ...
    if (isSuccessful) {
      ...
      if (isInteractive) {
        const serverUrls = prepareUrls('http', '0.0.0.0', 3000);
        const devServerUrls = prepareUrls('http', '0.0.0.0', 3200);
...
function createDevServerConfig({ publicPath }) {
  return {
...
async function waitForServerToStart({ port }) {
  ...
  if (!portFound) {
    console.log(
      chalk.red(
        `\nCouldn't find a server running on port ${chalk.bold(3000)}.`,
      ),
    );
    console.log(
      chalk.red(
