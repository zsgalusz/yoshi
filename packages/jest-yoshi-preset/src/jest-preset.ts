import fs from 'fs';
import { ProjectConfig, InitialOptions } from '@jest/types/build/Config';
import * as globs from 'yoshi-config/globs';
import loadJestYoshiConfig from 'yoshi-config/jest';
import { Config } from 'yoshi-config/build/jest/config';
import { pick } from 'lodash';
import chalk from 'chalk';
import globby from 'globby';
import { setupRequireHooks } from 'yoshi-helpers/require-hooks';
import { envs, supportedEnvs, withLatestJSDom } from './constants';
...
type ProjectNames = 'e2e' | 'spec';

type Mapping = {
  [key in ProjectNames]: keyof Config;
};

const projectOverrideMapping: Mapping = {
  e2e: 'e2eOptions',
  spec: 'specOptions',
};

const supportedProjectOverrideKeys: Array<keyof ProjectConfig> = ['globals'];

const supportedGlobalOverrideKeys: Array<keyof InitialOptions> = [
  'collectCoverage',
  'collectCoverageFrom',
  'coverageReporters',
  'coverageDirectory',
...
type CustomProjectConfig = Omit<Partial<ProjectConfig>, 'displayName'> & {
  displayName: ProjectNames;
};

const projects: Array<CustomProjectConfig> = [
  {
    displayName: 'spec',
...
const config: { projects: Array<Partial<CustomProjectConfig>> } & Omit<
  InitialOptions,
  'projects'
> = {
  ...
  projects: projects
    .filter(project => {
      if (envs) {
        return envs.includes(project.displayName);
      }
    ...
    .map(project => {
      ...
      // too
      const setupFilePaths: Array<string> = globby.sync(
