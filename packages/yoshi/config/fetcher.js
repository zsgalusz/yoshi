...
const { parse: parseAnsiColor } = require('ansicolor');
...
function parse(data, fileName) {
  if (data.type === 'httpFunctionResult') {
    if (data.logs.length > 0) {
      console.groupCollapsed(`http function ${fileName}`);
...
export function httpFunctionsFetcher(endpoint, fileName, methodName) {
  return async function(...args) {
    return new Promise(function(resolve, reject) {
      ...
      xhr.open('POST', `${endpoint}/${fileName}`, true);
      xhr.setRequestHeader('Accept', 'application/json');
      xhr.setRequestHeader('Content-Type', 'application/json');
      xhr.send(JSON.stringify(toJSON({ args, methodName })));
    }).catch(error => {
      console.log(error);
    });
  };
